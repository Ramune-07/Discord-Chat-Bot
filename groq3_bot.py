...
# 取り出した鍵を変数（箱）に入れます
DISCORD_TOKEN = os.getenv("DISCORD_TOKEN_GROQ3")
...
# 履歴ファイルの保存先ディレクトリ
HISTORY_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "chat_history", "groq3")
...
CHARACTER_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "characters", "groq3.txt")
try:
    with open(CHARACTER_FILE, "r", encoding="utf-8") as f:
        CHARACTER_SETTING = f.read()
except FileNotFoundError:
    print("警告: characters/groq3.txt が見つかりません。デフォルト設定を使用します。")
...
async def on_message(message):
    ...
    # 環境変数から特定チャンネルIDを取得
    SPECIFIC_CHANNEL_ID = os.getenv("CHANNEL_ID_GROQ3")
